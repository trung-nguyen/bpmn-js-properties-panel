...
var textField3 = function(translate, options, defaultParameters) {
  ...
  resource.html =
    domify('<label for="camunda-' + escapeHTML(resource.id) + '" style="display:none" "' + 
    ...
    '<div class="bpp-field-wrapper" ' +
      ...
      (canBeHidden ? 'data-show="isHidden"' : '') +
      '>' +
      '<input id="camunda-' + escapeHTML(resource.id) + '" type="text" name="' + escapeHTML(options.modelProperty) + '" placeholder="Hạn bảo hành"' +
        (canBeDisabled ? 'data-disable="isDisabled"' : '') +
        (canBeHidden ? 'data-show="isHidden"' : '') +
        ' />' +
