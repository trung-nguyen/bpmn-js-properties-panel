...
var TABLE_ROW_DIV_SNIPPET = '<div class="bpp-field-wrapper bpp-table-row " style="display:flex;">';
  ...
  forEach(properties, function (prop, idx) {
    if (idx === 0) {
      template += '<input  class="bpp-table-row-columns-' + columns + ' ' +
        ...
        '<select name="operator" id="operator" style="width: 20%;text-align: center;">' +
        '<option value="lon">></option>' +
        '<option value="nho"><</option>' +
        '<option value="bang">=</option>' +
        '<option value="lon-bang">>=</option>' +
        '<option value="nho-bang"><=</option>' +
        '</select>';
    }
    else {
      template += '<input class="bpp-table-row-columns-' + columns + ' ' +
...
function createLabelTemplate(labels) {
  ...
  forEach(labels, function (label) {
    if (label === 'Trường') {
      template += '<label class="bpp-table-row-columns-' + columns + ' bpp-lable-row">' + escapeHTML(label) + '</label>' +
        '<label class="bpp-table-row-columns-' + columns + ' bpp-lable-row operator">' + 'Toán tử' + '</label>';
...
module.exports = function(translate, options) {
  ...
  var html = domify((canAdd ?
    '<div class="bpp-table-add-row c__hide" ' + (canBeShown ? 'data-show="show"' : '') + '>' +
    ...
    '</div>' +
    '<button class="save-property">Lưu</button>'+
        '</div>');
  const s_property = document.querySelector('.save-property');
  console.log("sp", s_property);
